...
echo "$($PSQL "SELECT name FROM games INNER JOIN teams AS win_team ON games.winner_id=win_team.team_id WHERE year=2014 AND round='Eighth-Final' UNION SELECT name FROM games INNER JOIN teams AS oppo_team ON games.opponent_id=oppo_team.team_id WHERE year=2014 AND round='Eighth-Final' ORDER BY name")"
